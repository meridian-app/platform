defmodule PlatformWeb.UsersLoginLiveTest do
  ...
  describe "users login" do
    test "redirects if users login with valid credentials", %{conn: conn} do
      ...
      {:ok, lv, _html} = live(conn, ~p"/identity/login")

      form =
        form(lv, "#login_form",
          users: %{email: users.email, password: password, remember_me: true}
        )
  ...
  describe "login navigation" do
    ...
    test "redirects to forgot password page when the Forgot Password button is clicked", %{
      ...
      {:ok, conn} =
        ...
        |> render_click()
        |> follow_redirect(conn, ~p"/identity/reset_password")
