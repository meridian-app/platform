defmodule PlatformWeb.UsersSettingsLiveTest do
  ...
  describe "Settings page" do
    ...
    test "redirects if users is not logged in", %{conn: conn} do
      assert {:error, redirect} = live(conn, ~p"/identity/settings")

      assert {:redirect, %{to: path, flash: flash}} = redirect
      assert path == ~p"/identity/login"
  ...
  describe "update email form" do
    ...
    test "updates the users email", %{conn: conn, password: password, users: users} do
      ...
      {:ok, lv, _html} = live(conn, ~p"/identity/settings")
  ...
  describe "update password form" do
    ...
    test "updates the users password", %{conn: conn, users: users, password: password} do
      ...
      assert redirected_to(new_password_conn) == ~p"/identity/settings"
  ...
  describe "confirm email" do
    ...
    test "updates the users email once", %{conn: conn, users: users, token: token, email: email} do
      {:error, redirect} = live(conn, ~p"/identity/settings/confirm_email/#{token}")

      assert {:live_redirect, %{to: path, flash: flash}} = redirect
      assert path == ~p"/identity/settings"
    ...
    test "does not update email with invalid token", %{conn: conn, users: users} do
      {:error, redirect} = live(conn, ~p"/identity/settings/confirm_email/oops")
      assert {:live_redirect, %{to: path, flash: flash}} = redirect
      assert path == ~p"/identity/settings"
      assert %{"error" => message} = flash
      assert message == "Email change link is invalid or it has expired."
