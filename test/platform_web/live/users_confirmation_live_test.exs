defmodule PlatformWeb.UsersConfirmationLiveTest do
  ...
  describe "Confirm users" do
    test "renders confirmation page", %{conn: conn} do
      {:ok, _lv, html} = live(conn, ~p"/identity/confirm/some-token")
    ...
    test "confirms the given token once", %{conn: conn, users: users} do
      ...
      {:ok, lv, _html} = live(conn, ~p"/identity/confirm/#{token}")
    ...
    test "does not confirm email with invalid token", %{conn: conn, users: users} do
      {:ok, lv, _html} = live(conn, ~p"/identity/confirm/invalid-token")
