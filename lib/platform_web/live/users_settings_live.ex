defmodule PlatformWeb.UsersSettingsLive do
    ...
    <div class="space-y-12 divide-y">
      ...
      <div>
        <.simple_form
          for={@password_form}
          id="password_form"
          action={~p"/identity/login?_action=password_updated"}
          method="post"
  ...
  def mount(%{"token" => token}, _session, socket) do
    ...
    {:ok, push_navigate(socket, to: ~p"/identity/settings")}
  end
    ...
    case Accounts.apply_users_email(users, password, users_params) do
      {:ok, applied_users} ->
        Accounts.deliver_users_update_email_instructions(
          applied_users,
          users.email,
          &url(~p"/identity/settings/confirm_email/#{&1}")
        )
